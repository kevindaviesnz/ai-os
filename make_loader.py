with open("include/os_loader.h", "w") as f:
    code = """#ifndef OS_LOADER_H
#define OS_LOADER_H
...
/* Removed static so syscall.c and kernel.c can link to these */
...
void loader_init(void) {
    ...
    for (uint32_t i = 0; i < bundle->cartridge_count && i < 8; i++) {
        ...
        if (!code_base || !stack_alloc) {
            kpanic("FATAL: Out of memory mapping cartridge!\\n");
            continue;
        }
